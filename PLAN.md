...
| **VN Smart-Money cluster** | ✅ 2026-06-26 — dòng tiền thông minh cổ phiếu VN qua **CafeF** (`cafef.service.ts`, không key; finfo-api bị chặn vì resolve IP private): alert khối ngoại / tự doanh theo ngưỡng tỷ + streak phiên (`Alert: HPG foreign buy 50 3p`), alert kỹ thuật EOD (volume/RSI/MA-cross), **Smart-Money Digest** tự gửi 15:30 mỗi phiên (cron `30 15 * * 1-5`) + VN alert cron 15:15. |
| **Point & Figure (#11)** | ✅ 2026-06-27 — `PnF: <ticker>` dựng đồ thị P&F (`pnf.service.ts`, thuần toán: box/reversal, double-top/bottom, vertical-count target) render thành **PNG** (`pnf-image.ts` qua `@napi-rs/canvas`, font-free: X/O vector + chữ số 7-đoạn vì container Railway không có font). Box guard chặn input phi lý. Pro. |
...
| **TA card (#A)** | ✅ 2026-06-30 — `TA: <ticker>` gộp 1 thẻ: giá+%, RSI(14), vị trí giá vs MA20/50/200, volume vs TB20, hỗ trợ/kháng cự (đỉnh/đáy 20 phiên), tín hiệu P&F, streak khối ngoại/tự doanh. Tái dùng 100% chỉ báo có sẵn, không cần nguồn dữ liệu mới. Pro. Branch `sprint-16-ta-card`. |
| **Cảnh báo danh mục (#C)** | ✅ 2026-07-01 — `Position TP: <ticker> <giá>` / `Position SL: <ticker> <giá>` (hoặc `off` để bỏ) đặt take-profit/stop-loss theo từng vị thế; cron mỗi phút kiểm tra giá live (`portfolio_positions.take_profit/stop_loss`, cột mới), báo 1 lần rồi tự xoá mốc. **EOD Portfolio Digest** tự gửi 16:00 hàng ngày: NAV + lãi/lỗ cho user có vị thế (digest-eligible + `canPortfolio`). Tái dùng 100% `portfolio_positions` + `marketRouter.getPrices`, không cần nguồn dữ liệu mới. Pro. |
| **Lịch sự kiện DN (#B)** | ✅ 2026-07-01 — Spike xong: `cafef.vn/du-lieu/Ajax/PageNew/LichSuKien.ashx?Symbol=<mã>` (khác host path với `datahistory` đã dùng) trả lịch cổ tức & tăng vốn JSON, không key, **có sẵn sự kiện tương lai đã công bố** (kiểm chứng: SAB có cổ tức tiền GDKHQ 27/07/2026 khi "hôm nay" là 01/07/2026) — khác hẳn trang `lich-su-kien.chn` cũ (ASP.NET WebForms postback, dữ liệu chết ở ~2022-2023, không dùng được). Lệnh `Events: <ticker>` xem lịch (sắp tới/gần đây); cron 08:10 hàng ngày tự nhắc khi có sự kiện đúng "hôm nay + 3 ngày" (không cần bảng dedupe — hôm sau tự hết khớp). Pro. |
...
|---|---|---|---|---|
| A | ~~**TA card — `TA: HPG`**~~ | ~~1 thẻ gộp: giá+%, RSI(14), vị trí MA20/50/200, volume vs TB20, hỗ trợ/kháng cự, tín hiệu P&F, streak NN/tự doanh~~ | ✅ **Done 2026-06-30** | ✅ |
| B | ~~**Lịch sự kiện DN**~~ | ~~Ngày GDKHQ, cổ tức tiền/CP, phát hành thêm; auto-push cho mã watchlist/portfolio (vd "HPG GDKHQ sau 3 ngày")~~ | ✅ **Done 2026-07-01** | ✅ |
| C | ~~**Cảnh báo danh mục (Portfolio)**~~ | ~~Digest P&L danh mục cuối phiên + alert khi vị thế chạm TP/SL~~ | ✅ **Done 2026-07-01** | ✅ |
...
> **Khuyến nghị thứ tự:** A (TA card, done) → C (portfolio alerts, done) → B (lịch sự kiện, done) → D (định giá cơ bản).
